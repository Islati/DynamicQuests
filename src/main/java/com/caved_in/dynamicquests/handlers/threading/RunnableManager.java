package com.caved_in.dynamicquests.handlers.threading;
...
public class RunnableManager {
	...
	public void registerSynchRepeatTask(String Name, Runnable Task, long DelayInTicks, long RepeatTimeInTicks) {
		if (!Tasks.containsKey(Name)) {
			Tasks.put(Name, this.Plugin.getServer().getScheduler().scheduleSyncRepeatingTask(this.Plugin, Task,
					DelayInTicks, RepeatTimeInTicks));
		}
	}

	public void registerASynchRepeatTask(String Name, Runnable Task, long DelayInTicks, long RepeatTimeInTicks) {
		if (!Tasks.containsKey(Name)) {
			Tasks.put(Name, this.Plugin.getServer().getScheduler().scheduleAsyncRepeatingTask(this.Plugin, Task,
					DelayInTicks, RepeatTimeInTicks));
		}
	}
