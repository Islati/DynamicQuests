...
public class QuestNPC extends Trait
{
	...
	public void onAttach()
	{
		if (!DynamicQuestHandler.isNpcInUse(this.getNPC().getId()))
		{
			...
			else if (questTypeNumber >= 49 && questTypeNumber <= 99) //Make Mob-Kill quest
			{
				...
				CommonUtils.messageCosole(String.format("Medium Hunting Quest generated for NPC %s requring %s kills of %s",npc.getName() + "/" + npc.getId(), mobKillQuest.getEntityData().getEntityAmount(),mobKillQuest.getEntityData().getEntityType().name()));
	...
	public void onNpcRightClick(NPCRightClickEvent event)
	{
		if (event.getNPC() == this.getNPC()) //Make sure we're only handling for the npc that was clicked.
		{
			...
			DynamicQuestType questType = DynamicQuestHandler.getQuestType(questForNpc); //Get questType for the npc's quest
			if (player.getGameMode() != GameMode.CREATIVE)
			{
				...
				if (!questPlayer.isPlayerOnQuest(questForNpc))
				{
					...
					switch (questType)
					{
						case GATHER_MATERIAL:
							...
							player.sendMessage("Collect " + collectQuest.getQuestMaterial().getQuestMaterialAmount() + " of " + collectQuest.getQuestMaterial().getQuestMaterial().getItemType().name() + " plz");
							break;
						case KILL_MOB:
							MobKillQuest mobKillQuest = DynamicQuestHandler.getMobKillQuest(questForNpc);
							player.sendMessage("Kill " + mobKillQuest.getEntityData().getEntityAmount() + " of " + mobKillQuest.getEntityData().getEntityType().name());
							break;
						default:
				...
				else
				{
					...
					player.sendMessage("Your progress: [" + questPlayer.getQuestProgress(questForNpc).getPlayerProgress() + "] out of the required [" + questPlayer.getQuestProgress(questForNpc).getAmountRequired() + "] for quest type [" + questType.name() + "]");
				}
			}
			else
			{
